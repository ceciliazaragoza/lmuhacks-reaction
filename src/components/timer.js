...
function Timer() {
  ...
  return (
    <div>
      ...
      <div>
        <h1>{isFocusTimer ? 'Focus Time Remaining' : 'Break Time Remaining'}: {isFocusTimer ? (focusMinutes < 10 ? `0${focusMinutes}` : focusMinutes) : (breakMinutes < 10 ? `0${breakMinutes}` : breakMinutes)}:{isFocusTimer ? (focusSeconds < 10 ? `0${focusSeconds}` : focusSeconds) : (breakSeconds < 10 ? `0${breakSeconds}` : breakSeconds)}</h1>
      </div>
      <div>
